...
public:
    ...
    bool start_video_streaming(uint32_t width, uint32_t height, uint32_t fps) {
    ...
    bool start_audio_streaming(uint32_t sample_rate, uint32_t channels) {
        if (!initialized_) return false;
    ...
    bool initialize_input_handling() {
        if (!initialized_) return false;
    ...
    bool send_video_frame(const void* frame_data, size_t frame_size, uint32_t timestamp) {
        if (!initialized_) return false;
    ...
    bool send_audio_data(const void* audio_data, size_t data_size, uint32_t timestamp) {
        if (!initialized_) return false;
    ...
    bool send_mouse_input(int x, int y, uint8_t buttons) {
        if (!initialized_) return false;
    ...
    bool send_keyboard_input(uint32_t key_code, bool pressed) {
        if (!initialized_) return false;
    ...
    void get_video_stats(uint64_t& frames_sent, uint32_t& width, uint32_t& height, uint32_t& fps) {
